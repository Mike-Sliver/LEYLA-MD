...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
   ...
   let tee = `✳️ enter text\n\n📌 exapmle  : *${usedPrefix + command}* hola`
   let too = `✳️  enter text *+* \n\n📌 example : \n*${usedPrefix + command}* new *+* Bot`
    m.react(rwait)
...
switch (type) {
   ...
   let wo = global.API('fgmods', '/api/textpro/logowolf2', { text: 'FG98', text2: text}, 'apikey')
	...
	conn.sendFile(m.chat, ph, 'logo.png', `✅ 𝚁𝙴𝚂𝚄𝙻𝚃`, m)
	m.react(done)
	break 
	default:
} 
